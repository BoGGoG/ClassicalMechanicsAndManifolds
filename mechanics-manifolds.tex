...
\begin{defn}[section of a bundle]
    ...
    with $\pi\circ\sigma = \mathrm{Id}_\M$.
\end{defn}
\begin{center}
\begin{tikzpicture}
    \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
...
\end{center}
\begin{note}
In figure~\ref{fig:cicletangentbundle} a picture for a specific bundle is given.
...
\begin{figure}[tbh]
    ...
    \end{tikzpicture}
    \caption{Picture for a bundle, where $\M$ is a circle and $\mathcal{E}$ a cylinder. Picture modified from~\cite{texstackexchange:tangentbundlecircle}.}
    \label{fig:cicletangentbundle}
\end{figure}
\begin{note}

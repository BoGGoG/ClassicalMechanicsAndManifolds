...
\begin{defn}[Velocity]
    ...
    with
    \begin{equation}
        f \mapsto v_{\gamma,p}(f) := (f \circ \gamma)'(\gamma^{-1}(p))\,,
    \end{equation}
...
\begin{defn}[Tangent space]
    $\forall p \in \M$ the ``tangent space to $\M$ at $p$'' is
    \begin{equation}
...
\begin{proof} (Tangent space is a vector space)\\
    \begin{itemize}
        \item[2.]
            ...
            \end{align}
        \item [1.] Make a choice of chart $(U, x)$ with $p\in U$, later show independence of chart.
            Define
            \begin{align}
